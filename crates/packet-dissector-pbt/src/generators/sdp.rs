...
/// Free-form printable ASCII text (no CR/LF), at least one byte —
/// the RFC 8866, Section 9 ABNF `text` / `attribute-value` are
/// `byte-string = 1*(...)`, so empty values are not valid.
...
/// A `<proto>` value per the RFC 8866, Section 9 ABNF
/// `proto = token *("/" token)` — no empty, leading, or trailing slash
/// components.
fn arb_proto() -> impl Strategy<Value = String> {
    (
        "[A-Za-z0-9]{1,6}",
        prop::collection::vec("[A-Za-z0-9]{1,6}", 0..3),
    )
        .prop_map(|(first, rest)| {
            let mut proto = first;
            for part in rest {
...
pub fn arb_valid_sdp_session() -> impl Strategy<Value = (Vec<u8>, usize)> {
    (
        ...
        // s= (RFC 8866, Section 5.3) — MUST NOT be empty
        arb_text().prop_map(|s| format!("s={s}\r\n")),
        // c= (RFC 8866, Section 5.7) — a session description MUST contain
        // either a session-level c= or one in every media description;
        // always emit a session-level one to stay valid
        arb_token().prop_map(|addr| format!("c=IN IP4 {addr}\r\n")),
        ...
        .prop_map(
            |(origin, session_name, connection, timing, attrs, medias)| {
